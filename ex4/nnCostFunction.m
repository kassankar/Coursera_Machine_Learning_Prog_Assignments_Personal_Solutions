...
a3           = sigmoid(z3.');
% [val, ind] = max(a3, [], 2);
% h(sub2ind(size(h),ind.',1:numel(ind.')))=1;
J      = 1/m*sum(sum(-y.*log(a3.')-(1-y).*log(1-a3.'),1),2) + lambda/(2*m)*(sum(sum((Theta1(:,2:end)).^2,2),1) +sum(sum((Theta2(:,2:end)).^2,2),1));

gamma3 = a3 - y.';
gamma3 = gamma3.';
gamma2 = (Theta2.')*gamma3.*[ones(1,m); sigmoidGradient(z2)];
gamma2 = gamma2(2:end,:);
